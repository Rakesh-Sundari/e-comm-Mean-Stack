...
@Component({
  selector: 'app-home',
  imports: [MatButtonModule, CarouselModule, CommonModule, ProductCardComponent],
  ...
  ngOnInit() {
    ...
    this.customerService.getCategories().subscribe((result: Category[]) => {
      this.categories = (result || []).map(category => ({
        ...
        relatedImages: getRelatedImages(category.name)
      }));
    });

    function getRelatedImages(name: string): string[] {
